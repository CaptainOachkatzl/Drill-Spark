...
use xs_bevy_core_2d::{patterns::*, Grid, TodoList};
...
pub fn reveal_area<'a>(
    ...
    center: xs_bevy_core_2d::Position,
...
pub fn send_newly_revealed_tiles(
    ...
    q_tiles.for_each_mut(|mut tile| {
        tile.3 .0.get_new().iter().for_each(|&player| {
            collect_updated_tiles(&mut tile_updates, &q_players, player, (tile.1 .0, Some(*tile.2)));
        });
    });
...
fn collect_updated_tiles(
    player_updates: &mut HashMap<ConnectionId, Vec<(xs_bevy_core_2d::Position, Option<TileStatus>)>>,
    q_players: &Query<(&Player, &MiningQueue)>,
    player: Entity,
    tile: (xs_bevy_core_2d::Position, Option<TileStatus>),
) {
...
fn send_batched_tile_updates(
    net: &NetworkServer,
    tile_updates: HashMap<ConnectionId, Vec<(xs_bevy_core_2d::Position, Option<TileStatus>)>>,
) {
