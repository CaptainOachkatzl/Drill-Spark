...
use drillspark_common_lib::game_component::TileType;
use xs_bevy_core_2d::{
  Grid, Position, path_exists,
};
...
impl MiningQueue {
  ...
  fn next_mineable(&self, just_finished: Option<Entity>, mineable_params: IsMineableParams) -> Option<Entity> {
    let &next = self
      .queue
      .iter()
      .find(|&x| self.is_mineable(*x, just_finished, mineable_params))?;
    Some(next)
  }
  ...
  fn is_mineable(
    ...
    if !tile_type.is_tile_type_minable() || position == warpgate_position {
      return false;
    }
    ...
    let is_pathable_tile = |entity| {
      ...
      let position = get_position(entity);
      if position == warpgate_position {
        return true;
      }

      match get_tile_type(grid.get_value_by_position(get_position(entity)).unwrap()) {
        TileType::Ground | TileType::Building(_) => true,
        _ => false,
      }
    };
    path_exists(grid, get_position(entity), warpgate_position, &is_pathable_tile)
  }
}
