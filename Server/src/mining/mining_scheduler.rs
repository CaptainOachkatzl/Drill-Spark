...
pub fn update_mine_scheduler(
    ...
    q_player_systems.for_each_mut(|(player_entity, mut mining_queue, mut resource_store, &spawn_point, &id)| {
        ...
        let mineable_params: IsMineableParams = (&grid, spawn_point, &get_position, &get_tile_type, &is_revealed, &is_owned);
        if let Some(finished_tile) = mining_queue.update(time.delta(), mineable_params) {
            ...
            *finished_tile.0 = TileStatus::new(TileType::Ground, false, Some(id.0));
            mined_blocks.insert(finished_tile.1 .0, player_entity);
        }
